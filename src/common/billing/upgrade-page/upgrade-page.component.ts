...
export class UpgradePageComponent implements OnInit {
    ...
    public nextStep() {
        ...
        //console.log(this.state.mode,'====',this.stepper.selectedIndex,'//////',Steps.Period);
        if (this.state.mode === 'changePlan') {
            //console.log(this.state.mode,'====',this.stepper.selectedIndex,'//////',Steps.Period);
            this.changePlan();
    ...
    public onCompleted() {
        ...
        this.settings.isUser$.next(true);
    ...
    public changePlan() {
        ...
        if (activeSubscription.gateway === 'paypal') {
            this.paypalSubscriptions.changePlan(activeSubscription, plan).then(user => {
                ...
                this.router.navigate(['/web/billing/subscription']);
