...
export class CreditCardFormComponent implements OnDestroy, AfterViewInit {
    ...
    selectedPlan: any;
    promocodes$ = new BehaviorSubject<any>([]);
    selectedPromo:any=0;
    constructor(
        ...
        this.resetForm();
        this.state.selectedPlan$.subscribe(res=>{
            this.selectedPlan=res;
            if(this.selectedPlan){
                this.subscriptions.getPromos(this.selectedPlan.id).subscribe(res=>{
                    this.promocodes$.next(res);
                });   
            }
        })
    }
    ...
    public addCardToUser(stripeToken: stripe.Token) {
        this.loading$.next(true);
        this.subscriptions.addCard(stripeToken.id, this.currentUser.getModel().id)
            .pipe(finalize(() =>  this.loading$.next(false)))
            .subscribe(response => {
                this.resetForm();
                this.subscriptions.selectedPromo=this.selectedPromo;
