...
export class CreateSubscriptionPanelComponent {
    ...
    public subscribeOnStripe() {
        ...
        this.subscriptions.createOnStripe(this.getNewSubscriptionPayload(), this.currentUser.getModel().id)
            .subscribe(response => {
     ...
     */
     private getNewSubscriptionPayload(): NewSubscriptionPayload {
        const payload = {plan_id: this.plan.id, coupon_id: this.subscriptions.selectedPromo};
     //    const payload = {plan_id: this.plan.id, start_date: this.datePipe.transform(new Date(), 'dd-MM-yyyy')};
     let type = this.plan.interval
     var date = moment();
     let planDate
     if(type == 'day'){
        planDate = date.add(this.plan.interval_count, 'days');
    }
    if(type == 'week'){
        planDate = date.add(this.plan.interval_count, 'weeks');
    }
    if(type == 'month'){
        planDate = date.add(this.plan.interval_count, 'months'); // now + 5 months
    }
    if(type == 'year'){
        planDate= date.add(this.plan.interval_count, 'years'); // now + 5 months
    }
    payload['start_date'] = planDate.format('DD-MM-yyyy');
 
       // if (this.from) {
           // payload['start_date'] = this.from.renews_at;
           //payload['start_date'] = this.datePipe.transform(new Date(), 'dd-MM-yyyy');
       // }
